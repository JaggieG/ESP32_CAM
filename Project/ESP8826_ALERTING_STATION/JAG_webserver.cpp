...
JAG_webserver::JAG_webserver(int port, bool logSerial)
{
  ...
  ALERT_RECEIVED_PERSON_ = false;
  ALERT_RECEIVED_CAR_ = false;
  ALERT_RECEIVED_ANIMAL_ = false;
...
bool JAG_webserver::getAlertStatusPERSON() {
  return ALERT_RECEIVED_PERSON_;
}

bool JAG_webserver::getAlertStatusCAR() {
  return ALERT_RECEIVED_CAR_;
}

bool JAG_webserver::getAlertStatusANIMAL() {
  return ALERT_RECEIVED_ANIMAL_;
}

void JAG_webserver::setAlertStatusPERSON(bool ALERT_RECEIVED) {
  ALERT_RECEIVED_PERSON_ = ALERT_RECEIVED;
}

void JAG_webserver::setAlertStatusCAR(bool ALERT_RECEIVED) {
  ALERT_RECEIVED_CAR_ = ALERT_RECEIVED;
}

void JAG_webserver::setAlertStatusANIMAL(bool ALERT_RECEIVED) {
  ALERT_RECEIVED_ANIMAL_ = ALERT_RECEIVED;
}
...
void JAG_webserver::setupRoutes() {
  webserver.on("/", [&](){
   ...
   webserver.on("/buzzerPerson", [&](){
     ALERT_RECEIVED_PERSON_ = true;
     millisAtLastAlert = millis();
     webserver.send(200, "text/html", createHTMLpageWithContentRaw_("200"));
  });

   webserver.on("/buzzerAnimal", [&](){
     ALERT_RECEIVED_ANIMAL_ = true;
     millisAtLastAlert = millis();
     webserver.send(200, "text/html", createHTMLpageWithContentRaw_("200"));
  });

   webserver.on("/buzzerCar", [&](){
     ALERT_RECEIVED_CAR_ = true;
     millisAtLastAlert = millis();
     webserver.send(200, "text/html", createHTMLpageWithContentRaw_("200"));
  });
